...
class Solution {
    ...
    // Method to find the height of each node 
    // and populate the max heights at each level
    private int findHeight(TreeNode root, int l) {
        ...
        level[root.val] = l;
        height[root.val] = Math.max(findHeight(root.left, l + 1), 
                                findHeight(root.right, l + 1)) + 1;
